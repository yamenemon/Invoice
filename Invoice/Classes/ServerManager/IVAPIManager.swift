...
open class AlamofireRequestBuilder<T>: RequestBuilder<T> {
     ...
     */
    open func makeRequest(manager: SessionManager, method: HTTPMethod, encoding: ParameterEncoding, headers: HTTPHeaders) -> DataRequest {
        ...
        if fileKeys.count > 0 {
            ...
            if let param = self.parameters {
                ...
                if (param.keys.count > 0) {
                    ...
                    urlComponents!.queryItems = queryItems + param.map({ key, value in URLQueryItem(name: key, value: "\(value)") })
                    self.URLString = (urlComponents!.url?.absoluteString)!
                    
                }
            }
            
            let request = makeRequest(manager: manager, method: xMethod!, encoding: encoding, headers: self.headers)
